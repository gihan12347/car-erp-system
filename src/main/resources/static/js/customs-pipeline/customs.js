(function () {
    ...
    var pageStates = {
        ...
        4: { previewUrl: null, previewIsPdf: false, previewName: "" }
    };
    ...
    function getChassisNo() {
        ...
        var match = window.location.pathname.match(/\/(?:customs|declaration|assessment|worksheet)\/([^/]+)/);
    ...
    function switchTab(tab) {
        ...
        if (window.scrollImportPipelineToCurrent) {
            window.scrollImportPipelineToCurrent("smooth");
        }
    ...
    function initTabs() {
        ...
        var page4Stored = document.getElementById("page4StoredName");
        ...
        if (page4Stored && page4Stored.value) {
            updateTabBadge(4, true);
        }
        if (page4Stored && page4Stored.value && (!page2Stored || !page2Stored.value) && (!page3Stored || !page3Stored.value)) {
            switchTab(4);
        } else if (page3Stored && page3Stored.value && (!page2Stored || !page2Stored.value)) {
            switchTab(3);
        } else {
            switchTab(2);
        }
        ...
        if (/\/assessment(\/|$)/.test(path)) {
            requestedTab = 3;
        } else if (/\/worksheet(\/|$)/.test(path)) {
            requestedTab = 4;
        } else if (/\/declaration(\/|$)/.test(path)) {
            requestedTab = 2;
        }
        if (requestedTab === 2 || requestedTab === 3 || requestedTab === 4) {
    ...
    var JEVIC_FIELDS = [
        "jevicCertificateNo", "jevicIssueDate", "jevicLocation",
        "jevicMake", "jevicModel", "jevicEngineCapacity", "jevicFirstRegistration",
        "jevicChassisVin", "jevicEngineNo", "jevicCurrentOdometer",
        "jevicInspectionDate", "jevicRemarks"
    ];
    ...
    function fillForm(result, page) {
        if (page === 1) {
            ...
            WORKSHEET_FIELDS.forEach(function (fieldId) {
                var el = document.getElementById(fieldId);
                if (el) {
                    el.value = "";
                }
