(function () {
    ...
    var documentOriginalNameInput = document.getElementById("page1OriginalName");
    var documentStoredNameInput = document.getElementById("page1StoredName");
    var documentContentTypeInput = document.getElementById("page1ContentType");
    ...
    function buildDocumentUrl(storedName) {
        ...
        return "/jevic/documents/" + encodeURIComponent(storedName);
    }
    ...
    function uploadDocument(file, onSuccess, onError) {
        ...
        xhr.open("POST", "/jevic/upload-document");
        xhr.timeout = 120000;
        if (token && header) {
    ...
    function fillForm(result) {
        ...
        var ocr = document.getElementById("ocrTextPage1");
    ...
    function autoFill() {
        ...
        withParseFile(function (file) {
            ...
            var header = document.querySelector('meta[name="_csrf_header"]');
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/jevic/parse-document");
            xhr.timeout = 180000;
            if (token && header) {
                xhr.setRequestHeader(header.content, token.content);
            }
