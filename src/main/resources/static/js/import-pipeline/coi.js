(function () {
    ...
    var objectUrl = null;
    var FIELD_ORDER = [
        "certificateNo", "issueDate", "inspectionBranch", "make", "model",
        "engineCapacity", "firstRegistration", "chassisVin", "engineNo",
        "inspectedMileage", "inspectionDate", "remarks"
    ];
    ...
    function buildDocumentUrl(storedName) {
        ...
        return "/coi/documents/" + encodeURIComponent(storedName);
    }
    ...
    function renderPreviewFrame(url, name, contentType) {
        ...
        markDocumentReady(name || "Certificate of inspection");
        if (previewEmpty) {
        ...
        previewUrl = url;
        previewName = name || "Certificate of inspection";
        if (previewIsPdf) {
            if (previewImage) {
    ...
    function openViewer() {
        ...
        if (!previewUrl) {
            setStatus("Upload a certificate of inspection first to preview it.", false);
            return;
        }
        if (window.DocumentViewer) {
            DocumentViewer.open({
                url: previewUrl,
                title: previewName || "Certificate of inspection",
                isPdf: previewIsPdf
            });
        }
    }
    ...
    function uploadDocument(file, onSuccess, onError) {
        ...
        xhr.open("POST", "/coi/upload-document");
        xhr.timeout = 120000;
        if (token && header) {
    ...
    function selectFile(file) {
        ...
        uploadDocument(file, function (result) {
            ...
            setStatus("Document saved. Click Auto fill to read fields from the certificate of inspection.", true);
        }, function (message) {
            selectedFile = null;
            updateActionButtons();
    ...
    function initExistingDocument() {
        ...
        setStatus("Saved certificate of inspection loaded. Use Auto fill to read fields from the document.", true);
        updateActionButtons();
    }
    ...
    function withParseFile(callback, onError) {
        ...
        if (!hasSavedDocument()) {
            onError("Upload a certificate of inspection first.");
            return;
        }
    ...
    function autoFill() {
        if (!canAutoFill()) {
            setStatus("Upload a certificate of inspection first.", false);
            return;
        }
        showLoader(true);
        setStatus("Reading certificate of inspection...", true);
        withParseFile(function (file) {
            ...
            var header = document.querySelector('meta[name="_csrf_header"]');
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/coi/parse-document");
            xhr.timeout = 180000;
            if (token && header) {
                xhr.setRequestHeader(header.content, token.content);
            }
