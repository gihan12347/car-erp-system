(function () {
    ...
    function buildDocumentUrl(storedName) {
        ...
        return "/grade/documents/" + encodeURIComponent(storedName);
    }
    ...
    function renderPreviewFrame(url, name, contentType) {
        ...
        markDocumentReady(name || "Grade search");
        if (previewEmpty) {
    ...
    function openViewer() {
        ...
        if (!previewUrl) {
            setStatus("Upload a grade search document first to preview it.", false);
            return;
        }
        if (window.DocumentViewer) {
            DocumentViewer.open({
                url: previewUrl,
                title: previewName || "Grade search",
                isPdf: previewIsPdf
            });
        }
    }
    ...
    function uploadDocument(file, onSuccess, onError) {
        ...
        xhr.open("POST", "/grade/upload-document");
        xhr.timeout = 120000;
        if (token && header) {
    ...
    function selectFile(file) {
        ...
        uploadDocument(file, function (result) {
            ...
            setStatus("Document saved. Click Auto fill to read fields from the grade search document.", true);
        }, function (message) {
            selectedFile = null;
            updateActionButtons();
    ...
    function initExistingDocument() {
        ...
        setStatus("Saved grade search document loaded. Use Auto fill to read fields from the document.", true);
        updateActionButtons();
    }
    ...
    function fillForm(result) {
        ...
        var chassisNoEl = document.getElementById("chassisNo");
        if (chassisNoEl && !chassisNoEl.value.trim() && fields.chassisNumber) {
            chassisNoEl.value = String(fields.chassisNumber).trim();
        }
        return filled;
    }
    ...
    function withParseFile(callback, onError) {
        ...
        if (!hasSavedDocument()) {
            onError("Upload a grade search document first.");
            return;
        }
    ...
    function autoFill() {
        if (!canAutoFill()) {
            setStatus("Upload a grade search document first.", false);
            return;
        }
        showLoader(true);
        setStatus("Reading grade search document...", true);
        withParseFile(function (file) {
            ...
            var header = document.querySelector('meta[name="_csrf_header"]');
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "/grade/parse-document");
            xhr.timeout = 180000;
            if (token && header) {
                xhr.setRequestHeader(header.content, token.content);
            }
